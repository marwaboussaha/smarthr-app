...
export default function ClientProfile() {
  ...
  const save = async e => {
    ...
    try {
      const r = await clientApi.updateProfile(user.id, form)
  ...
  const savePw = async e => {
    ...
    try {
      await clientApi.updateProfile(user.id, pwForm)
      ...
      <div className="card" style={{marginBottom:20}}>
        <div style={{display:'flex',alignItems:'center',gap:20,marginBottom:24}}>
          ...
          <div>
            ...
            <div style={{color:'var(--c-muted)',fontSize:'.8rem',marginTop:2}}>{p?.type ?? 'Client'}</div>
          </div>
      ...
      <Modal open={editing} onClose={()=>setEditing(false)} title="Edit Profile">
        <form onSubmit={save}>
          <div className="field-row">
            <Input label="First Name" value={form.firstname ?? form.name ?? ''} onChange={e=>setForm(f=>({...f,firstname:e.target.value}))}/>
